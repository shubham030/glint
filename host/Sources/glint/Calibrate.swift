...
func calibrateTouch(dev: Link, hello: Hello) {
    ...
    /* The arithmetic lives in GlintCore, pinned by tests against both panels'
     * measured corners — deriving it by eye is how it goes wrong. */
    let mapping = deriveMapping(
        topLeft: taps[0], topRight: taps[1], bottomLeft: taps[2])
    let flags = mapping.flags

    print("")
    print(
        "mapping: swapXY=\(mapping.swapXY) flipX=\(mapping.flipX) "
            + "flipY=\(mapping.flipY)")
    print(
