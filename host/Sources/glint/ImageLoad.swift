...
func renderRGB565(
    ...
    if satPct != 100 || conPct != 100 {
        /* Saturation/contrast lift in integer math — compensates the panel's
         * flat generic gamma until the firmware-side init table is tuned. */
        for i in 0..<(width * height) {
            ...
            func boost(_ c: Int) -> UInt16 {
                var v = avg + (c - avg) * satPct / 100
                v = 128 + (v - 128) * conPct / 100
